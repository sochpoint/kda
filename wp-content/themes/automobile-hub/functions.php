...
function automobile_hub_scripts() {
	...
	if(!wp_is_mobile()){
		...
		wp_enqueue_script( 'automobile-hub-superfish-custom-scripts', esc_url( get_template_directory_uri() ) . '/assets/js/automobile-hub-superfish-custom.js', array('jquery','jquery-superfish'), true );
	}
...
if ( ! function_exists( 'automobile_hub_credit' ) ) {
	function automobile_hub_credit(){
		echo "<a href=".esc_url(AUTOMOBILE_HUB_CREDIT)." target='_blank'>".esc_html__('Automobile WordPress Theme','automobile-hub')."</a>";
	}
}
